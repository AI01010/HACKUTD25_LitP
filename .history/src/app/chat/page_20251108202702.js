	...
	async function handlePdfFile(file) {
		...
		try {
			const res = await fetch("/api/upload", {
				...
				// - Integration with the upload API: the upload handler will post a PDF to
				//   `/api/upload` and use any extracted text as a user message (see other
				//   files for the upload route implementation).
				export default function ChatPage() {
					...
					const [input, setInput] = useState("");
					// isTyping & streamingText: used to show the bot composing/typing
					const [isTyping, setIsTyping] = useState(false);
					const [streamingText, setStreamingText] = useState("");
					// isSending: disables the send button while bot is composing
					const [isSending, setIsSending] = useState(false);

					// Refs used for scrolling, speech recognition, and TTS buffering
					const bottomRef = useRef(null);
					const recognitionRef = useRef(null);
					const [recognizing, setRecognizing] = useState(false);
					const [speechSupported, setSpeechSupported] = useState(() => typeof window !== 'undefined' && (window.SpeechRecognition || window.webkitSpeechRecognition) && 'speechSynthesis' in window);
					...
					useEffect(() => {
						// Keep the view scrolled to the latest message whenever messages or
						// streaming text changes.
						bottomRef.current?.scrollIntoView({ behavior: "smooth", block: "end" });
					}, [messages, streamingText, isTyping]);

					// Helper: add a message to the conversation state
					function addMessage(sender, text) {
						setMessages((m) => [...m, { id: Date.now() + Math.random(), sender, text, time: new Date().toISOString() }]);
					}

					// handleSend: sends either the provided `raw` text or the textarea input.
					// It pushes a user message and triggers the bot response.
					function handleSend(raw = null) {
						const text = (raw ?? input).trim();
						if (!text) return;
						// push user message
						addMessage("user", text);
						setInput("");
						// trigger bot behavior that streams a response
						simulateBotResponse(text);
					}

					// Simulated bot: this is a small deterministic function to produce
					// plausible replies for the demo. It streams characters into
					// `streamingText` to show typing animation, and on completion pushes the
					// final bot message into the `messages` array.
					function simulateBotResponse(prompt) {
						setIsSending(true);
						setIsTyping(true);
						setStreamingText("");

						// Choose a canned response based on a few keyword heuristics
						...
						// Stream the response character-by-character to create a typing
						// effect; simultaneously buffer text for TTS playback.
						let i = 0;
						const interval = setInterval(() => {
							...
							// when done, finalize the response and push a bot message
					...
					// speak: simple wrapper around the Web Speech API (speechSynthesis)
					...
					// toggleMic: start/stop speech recognition if available in the browser
					function toggleMic() {
